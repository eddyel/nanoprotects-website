...
// ===========================================
// DONNÉES PAYS (version simplifiée avec les plus courants)
// ===========================================
...
function ToggleButton({ label, selected, onToggle }: ToggleButtonProps) {
  return (
    <button
      ...
      className={`px-4 py-2 min-h-[44px] rounded-lg border-2 transition-all text-sm font-medium active:scale-[0.98] flex items-center justify-center gap-1 ${
        selected
...
export default function Contact() {
  ...
  const handlePhoneChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Limiter aux chiffres et à la longueur attendue
    const digits = raw.replace(/\D/g, '').slice(0, expectedDigits);
  ...
  const toggleMateriau = (materiau: string) => {
    ...
    setSelectedMateriaux(prev =>
      prev.includes(materiau) ? prev.filter(m => m !== materiau) : [...prev, materiau]
    );
  };
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.phone.trim()) {
      ...
      if (digits.length !== expectedDigits) {
        newErrors.phone = `Le numéro doit contenir ${expectedDigits} chiffres pour ${selectedCountry.name}`;
      }
    }
    ...
    if (isAutreSelected && !formData.autreMateriau.trim()) {
      newErrors.autreMateriau = 'Veuillez préciser le matériau';
    }
            ...
            <form onSubmit={handleSubmit} className="space-y-8" noValidate>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <div className="relative w-40" ref={dropdownRef}>
                    <button
                      ...
                      className="w-full h-[44px] px-3 py-2 border border-gray-300 rounded-lg bg-white hover:bg-gray-50 focus:ring-2 focus:ring-primary focus:border-transparent flex items-center justify-between"
                    >
                        ...
                        <div className="max-h-64 overflow-y-auto">
                          {filteredCountries.map(country => (
                            <button
                              ...
                              className={`w-full flex items-center gap-3 px-3 py-2 text-left hover:bg-amber-50 ${
                ...
                <p className="text-gray-500 text-xs mt-1">
                  {selectedCountry.code} ({expectedDigits} chiffres)
                </p>
              </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {protections.map(prot => (
                    <ToggleButton
                      ...
                      onToggle={() => toggleProtection(prot)}
                    />
                  ))}
                </div>
              </div>
              ...
              <Button
                ...
                className="w-full text-lg py-6 btn-brand disabled:opacity-50"
              >
